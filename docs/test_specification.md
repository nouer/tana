...
| 種別 | ファイル | テスト数 |
|------|---------|---------|
| ユニットテスト | tana.calc.test.js | 126件 |
| E2Eテスト（単体機能） | e2e.test.js | 88件 |
| E2Eテスト（ワークフロー） | e2e.workflow.test.js | 39件 |
| **合計** | | **253件** |

### テストID体系
- UT-XXX-NNN: ユニットテスト（XXX=カテゴリ、NNN=連番）
- E2E-XXX-NNN: E2Eテスト 単体機能（XXX=カテゴリ、NNN=連番）
- WF-UCn-NNN: E2Eテスト ワークフロー（n=ユースケース番号、NNN=連番）
...
| UT-VP-014 | nameKanaにカタカナ → invalid | valid=false, "ふりがなに使用できない文字が含まれています" |
| UT-VP-015 | nameKanaにひらがな → valid | valid=true |
| UT-VP-016 | nameKanaに数字を含む → valid | valid=true |
| UT-VP-017 | nameKanaに英字・ハイフンを含む → valid | valid=true |
| UT-VP-018 | nameKanaに中黒を含む → valid | valid=true |
| UT-VP-019 | nameKanaに漢字 → invalid | valid=false, "ふりがなに使用できない文字が含まれています" |
...
| E2E-TXN-007 | 取引履歴 商品フィルター | 商品選択で絞り込み → 該当商品の取引のみ表示 |
...
| E2E-RPT-009 | 在庫レポート カテゴリフィルター | 消耗品選択→消耗品のみ表示、件数が全件より少ない |
| E2E-RPT-010 | 在庫レポート 並び替え | 在庫数昇順/降順でソートが正しく適用される |
| E2E-RPT-011 | 入出庫履歴レポート 商品フィルター | 商品選択→該当商品の取引のみ表示 |
| E2E-RPT-012 | 入出庫履歴レポート 種別フィルター | 入庫選択→「入庫」種別の取引のみ表示 |
...
### 2.14 構造的整合性ガード (E2E-SIG)

| ID | テスト内容 | 期待結果 |
|----|-----------|---------|
| E2E-SIG-001 | 全ボタンにイベントリスナーが存在する | onclick/addEventListener付きのボタンが全て機能する |
| E2E-SIG-002 | 全getElementById呼び出しのIDがDOMに存在する | script.jsの全IDがindex.htmlに存在 |
| E2E-SIG-003 | 全参照リソースが正常にロードされる | CSS/JS/画像等の外部リソースが全て200応答 |
...
ワークフローE2Eテストは、個別機能のテスト（e2e.test.js）とは異なり、複数の機能を組み合わせた実際のユースケースを検証する。「さくらクリニック」という架空の鍼灸院を舞台とした8つのシナリオを通じて、アプリの全機能がユーザーの日常業務フローの中で正しく動作することを確認する。

- **テストファイル**: `local_app/e2e.workflow.test.js`
- **ヘルパーモジュール**: `local_app/e2e.helpers.js`（`e2e.test.js` と共有）
- **ユースケース定義**: `docs/usecases.md`
- **テスト数**: 39件

### 3.1 UC1: 開業準備 — 初期セットアップ (WF-UC1)

| ID | テスト内容 | 期待結果 |
|----|-----------|---------|
| WF-UC1-001 | 事業者情報を入力・保存する | 保存後リロードしても情報が保持される |
| WF-UC1-002 | 在庫管理設定（期限アラート30日）を保存する | 設定がIndexedDBに保存される |
| WF-UC1-003 | 消耗品（セイリン鍼 J-15）を登録する | 商品が登録されカウント1 |
| WF-UC1-004 | 物販商品（グルコサミンサプリ）を登録する | 商品が登録されカウント2 |
| WF-UC1-005 | ダッシュボードに商品数が反映される | 登録商品数=2 |

### 3.2 UC2: 初回入荷 — 在庫の受け入れ (WF-UC2)

| ID | テスト内容 | 期待結果 |
|----|-----------|---------|
| WF-UC2-001 | 消耗品を入荷する（数量200、ロットLOT-2026A、期限2027-12-31） | 入荷取引が記録される |
| WF-UC2-002 | バーコードスキャンで物販を入荷する（数量20） | スキャンで商品特定、入荷記録 |
| WF-UC2-003 | ダッシュボードに在庫合計220が反映される | 総在庫数=220 |
| WF-UC2-004 | 在庫不足アラートが表示されない | アラートセクション空 |

### 3.3 UC3: 日常業務 — 消費と販売 (WF-UC3)

| ID | テスト内容 | 期待結果 |
|----|-----------|---------|
| WF-UC3-001 | 消耗品を使用する（数量30） | 使用取引記録、在庫170 |
| WF-UC3-002 | 物販を販売する（数量2） | 販売取引記録、在庫18 |
| WF-UC3-003 | ダッシュボードに在庫数が更新される（170、18） | 総在庫数=188 |
| WF-UC3-004 | minStockを200に変更すると在庫不足アラートが表示される | アラートにセイリン鍼表示 |
| WF-UC3-005 | 最近の取引に2件以上表示される | 取引サマリーに表示あり |

### 3.4 UC4: 期限管理 — 消費期限の確認と対応 (WF-UC4)

| ID | テスト内容 | 期待結果 |
|----|-----------|---------|
| WF-UC4-001 | ダッシュボードに期限アラートが表示される | 期限切れ/期限間近アイテム表示 |
| WF-UC4-002 | 期限レポートにロット別ステータスが表示される | テーブルにロット情報あり |
| WF-UC4-003 | 期限切れ・期限間近のバッジ色が正しい | expired=赤、warning=黄 |
| WF-UC4-004 | 期限管理OFFの商品が期限レポートに出ない | trackExpiry=false商品が非表示 |

### 3.5 UC5: 月次棚卸 — 在庫確認と調整 (WF-UC5)

| ID | テスト内容 | 期待結果 |
|----|-----------|---------|
| WF-UC5-001 | 新規棚卸を開始する | in_progressセッション作成 |
| WF-UC5-002 | テンキーで商品1の実数量165を入力する（差異-5） | actualQuantity=165, status=counted |
| WF-UC5-003 | テンキーで商品2の実数量20を入力する（差異+2） | actualQuantity=20, status=counted |
| WF-UC5-004 | 棚卸を完了する（確認ダイアログ承認） | status=completed、調整取引2件生成 |
| WF-UC5-005 | 棚卸履歴に完了済みセッションが表示される | 履歴リストに表示あり |
| WF-UC5-006 | 差異レポートでセッションを選択して差異が表示される | セッション選択、テーブル表示 |

### 3.6 UC6: レポート活用 — 経営分析 (WF-UC6)

| ID | テスト内容 | 期待結果 |
|----|-----------|---------|
| WF-UC6-001 | 在庫レポートに全商品一覧が表示される | テーブルに行あり |
| WF-UC6-002 | カテゴリフィルタで消耗品のみ/物販のみを表示する | フィルタ切替で件数変化 |
| WF-UC6-003 | ソートで在庫数昇順/降順を切り替える | ソート順が正しく反映 |
| WF-UC6-004 | 取引履歴レポートの種別フィルタ | 入庫のみ表示 |
| WF-UC6-005 | 期限レポートにロット情報とステータスバッジが表示される | テーブル・バッジ表示 |
| WF-UC6-006 | 差異レポートにセッションが表示される | セッション選択肢あり |

### 3.7 UC7: データ保全 — バックアップと復元 (WF-UC7)

| ID | テスト内容 | 期待結果 |
|----|-----------|---------|
| WF-UC7-001 | エクスポートでJSONが正しく生成される | appName=tana、データ件数確認 |
| WF-UC7-002 | 全データ削除で商品0件・取引0件になる | 全ストア空 |
| WF-UC7-003 | エクスポートしたデータをインポートして復元する | 件数が復元前と一致 |
| WF-UC7-004 | 復元後のダッシュボードが正常に表示される | 在庫概要が正常表示 |

### 3.8 UC8: 商品管理 — 検索・編集・削除 (WF-UC8)

| ID | テスト内容 | 期待結果 |
|----|-----------|---------|
| WF-UC8-001 | 商品名で検索してフィルタ結果を確認する | 部分一致で絞り込み |
| WF-UC8-002 | JANコードで検索して結果が表示される | 1件表示 |
| WF-UC8-003 | カテゴリフィルタで消耗品/物販を切り替える | フィルタ切替で件数変化 |
| WF-UC8-004 | 商品を編集して名前を変更する | DB上の商品名が更新 |
| WF-UC8-005 | 商品を削除する（論理削除、isActive=false） | 一覧から非表示、DBにisActive=false |
