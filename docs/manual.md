...
| 商品名（ふりがな）    | 任意     | 検索用のふりがな                                       | ひらがな・英数字・記号（・-ー） |
...
- 検索対象: 商品名、商品名（ふりがな）、商品コード、JAN コード
- 大文字・小文字を区別しません。
...
全商品のカウントが完了したら、棚卸を確定します。すべての商品をカウントしていなくても棚卸を完了できます。

**操作手順:**

1. 「棚卸を完了」ボタンをタップ
2. 確認ダイアログが表示される:「棚卸を完了しますか？差異がある商品は自動調整されます。」
3. 未カウントの商品がある場合、追加の確認ダイアログが表示される:「未カウントの商品が N 件あります。理論在庫数で棚卸しますか？」。「OK」を選択すると、未カウントの商品にはシステム上の理論在庫数が自動で入力されます。
4. 差異レポートが自動生成される:
   - 総品目数
   - カウント済み品目数
   - 差異のある品目数
   - 各商品の差異量（実数 - システム在庫数）
5. 差異のある商品には、自動で調整トランザクション（transactionType: adjust）が生成される
6. 調整トランザクションにより、システム在庫数が実数に合わせて更新される
7. 棚卸レコードのステータスが「completed」に変更される
...
- [**Tana を使う →**](index.html)
- [**活用事例を見る →**](usecases_showcase.html) — 実際の治療院での活用シーンを画像付きでご紹介
- [**Tana の紹介 →**](promotion.html) — 機能の概要・比較・導入ステップ
